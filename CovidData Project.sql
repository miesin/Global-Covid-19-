...
select location, max(total_cases) as Highest_Infected_Count, avg(population) as population, (max(total_cases)/avg(population))*100 as Percent_Population_infected 
from CovidDeaths
...
select date, sum(new_cases) as Total_Cases, sum(new_deaths) as Total_Deaths,
	total_deaths_worldwide(sum(new_deaths)/sum(new_cases))*100 as Deaths_per_cases
...
select sum(new_cases) as Total_Cases, sum(new_deaths) as Total_Deaths,
	(sum(new_deaths)/sum(new_cases))*100 as Deaths_per_cases_percent
-- , total_deaths, total_cases, (total_deaths/total_cases)*100 as DeathPercentage, 
from CovidDeaths 
where continent !=''
-- group by date
order by 1; -- date,Total_Cases; 

-- Using the two tables 

select * from CovidDeaths dea join CovidVaccinations vac
on dea.location=vac.location 
and dea.date=vac.d ate;
